...
sakai.editor.editors.ckeditor.launch = function(targetId, config, w, h) {
	...
	//Some defaults for audio recorder
        ...
        [
            ...
            (sakai.editor.enableResourceSearch
                ? ['warpwire','AudioRecorder','ResourceSearch', 'Image','Movie','Flash','Table','HorizontalRule','Smiley','SpecialChar','fmath_formula']
                : ['warpwire','AudioRecorder','Image','Movie','Flash','Table','HorizontalRule','Smiley','SpecialChar','fmath_formula']),
            '/',
    ...
    if (config) {
       ...
       // Warpwire has been enabled for this Sakai installation
       if( (typeof(ckconfig.warpwireURL) != 'undefined') && (ckconfig.warpwireURL.length > 0) && (ckconfig.warpwireURL != '[[WW_ROOT_DOMAIN]]') ) {
        // load the Warpwire plugin script
        CKEDITOR.plugins.addExternal('warpwire',basePath+'warpwire/', 'plugin.js');
        var exPluginTest = /\,$/;
        // add the Warpwire plugin to the extra plugins configuration string
        ckconfig.extraPlugins += ( exPluginTest.exec(ckconfig.extraPlugins) == null ? ',warpwire' : 'warpwire' );
       }
